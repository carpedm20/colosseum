...
def view_file_list(request, challenge_id=None):
    template = 'file/view_file_list.html'

    challenge = StudyGroup.objects.get(unique_id=challenge)
    file_list = []
    ...
    return render(request, template, {'challenge': challenge})#'file_list': file_list})
    

@require_POST
def upload(request, challenge_id=None, post_id=None):
    ...
    file_dict = {
        ...
        'deleteUrl': reverse('jfu_delete', kwargs = { 'challenge_id': challenge_id, 'pk': instance.pk, 'post_id': post_id }),
        'deleteType': 'POST',
    }
